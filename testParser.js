      ...
      if (symbolIdCol !== -1) {
        ...
        for (let c = symbolIdCol + 1; c < row.length; c++) {
          if (row[c]) {
            ...
            if (symId.includes("(B1)") || symId === "BONUS" || symId === "BOUNS(B1)") realSymId = "B1";
            if (symId.includes("(B2)") || symId === "SUPERBONUS") realSymId = "B2";
            if (symId.includes("Scatter") || symId.includes("(S1)")) realSymId = "S1";
            ...
            for (let r = 1; r <= 15; r++) {
              ...
              if (selfMatchStr && (!isNaN(parseInt(selfMatchStr)) || selfMatchStr.includes(">="))) {
                matchCountStr = selfMatchStr;
                payoutStr = selfPayoutStr;
              } else if (sharedMatchStr && (!isNaN(parseInt(sharedMatchStr)) || sharedMatchStr.includes(">="))) {
                matchCountStr = sharedMatchStr;
                payoutStr = sharedPayoutStr;
              }
              if (matchCountStr) {
                let matchValue = parseInt(matchCountStr.replace(/[^0-9]/g, ""));
                let payout = payoutStr === "--" || payoutStr === "-" || !payoutStr ? 0 : parseFloat(payoutStr) || 0;
                if (!isNaN(matchValue) && matchValue >= 2) {
                  if (matchCountStr.includes(">=")) {
                    for (let m = matchValue; m <= 30; m++) {
                      paytableMap[realSymId].payouts[`match${m}`] = payout;
                    }
                  } else {
                    paytableMap[realSymId].payouts[`match${matchValue}`] = payout;
                  }
                }
              }
            }
